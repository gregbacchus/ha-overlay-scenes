...
A new source evicts the previous source on the *same channel* only, not every channel the old source happened to target. For a source spanning multiple attributes (e.g. switch-on setting both `state` and `brightness`), eviction is evaluated independently per channel — the registry's exclusivity bookkeeping needs to be keyed at channel granularity, not layer granularity.

**Consequence worth designing for:** a multi-attribute source can end up partially evicted — one of its channels replaced by a newer source while another channel it still holds keeps running on its original timer. That's the correct behavior given this decision (each channel resolves independently), but it means the registry can't treat "a source" as a single atomic unit for lifecycle purposes — it needs to track per-channel occupancy even when several channels were originally activated together as one source layer.
